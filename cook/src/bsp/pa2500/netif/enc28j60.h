...
* All Rights Reserved
******************************************************************************/
#ifndef __CARIBOU_ENC28J60_H__
#define __CARIBOU_ENC28J60_H__
...
extern void     enc28j60_setmac             (uint8_t* mac);
...
extern uint16_t enc28j60_recv_packet        (uint8_t *buf, uint16_t buflen);

// R/W control registers
extern void 	enc28j60_set_bank	        (uint8_t adr);
extern void 	enc28j60_write_op	        (uint8_t cmd, uint8_t adr, uint8_t data);
extern uint8_t  enc28j60_rcr                (uint8_t adr);
extern void     enc28j60_wcr                (uint8_t adr, uint8_t arg);
extern uint16_t enc28j60_rcr16              (uint8_t adr);
extern void     enc28j60_wcr16              (uint8_t adr, uint16_t arg);
extern void     enc28j60_bfc                (uint8_t adr, uint8_t mask); // Clr bits (reg &= ~mask)
extern void     enc28j60_bfs                (uint8_t adr, uint8_t mask); // Set bits (reg |= mask)

// R/W Rx/Tx buffer
extern void     enc28j60_read_buffer        (uint8_t *buf, uint16_t len);
extern void     enc28j60_write_buffer       (uint8_t *buf, uint16_t len);

// R/W PHY reg
extern uint16_t enc28j60_read_phy           (uint8_t adr);
extern void     enc28j60_write_phy          (uint8_t adr, uint16_t data);

extern uint32_t enc28j60_interrupt_disable  (uint32_t level);
extern void     enc28j60_interrupt_enable   (uint32_t level);
extern bool     enc28j60_check_link_status  (void);
