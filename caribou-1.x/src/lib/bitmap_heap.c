...
#define HEAP_STATE (&heap_state[heap_num])
 ...
 */
static void* allocate(heap_state_t* heap_state, int32_t block, int32_t blocks)
{
	int32_t n;
	void* pointer=NULL;
	if ( valid(heap_state,block) )
 ...
 */
static bool deallocate(heap_state_t* heap_state, int32_t block, int32_t blocks)
{
	int32_t n;
	if ( valid(heap_state,block) )
	{
