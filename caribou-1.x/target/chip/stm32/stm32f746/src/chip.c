...
{
	isr_enter();
	kvector_isr((InterruptVector)(SCB->ICSR & SCB_ICSR_VECTACTIVE_Msk)-16);
	isr_exit();
}
...
int chip_vector_enable(uint32_t vector)
{
	...
	if ( vector < 32 )
	{
		NVIC->ICPR[0] = (1 << (uint32_t)vector);
		NVIC->ISER[0] = (1 << (uint32_t)vector);
	}
	else if ( vector < 64 && vector >= 32 )
	{
		NVIC->ICPR[1] = (1 << (uint32_t)(vector-32));
		NVIC->ISER[1] = (1 << (uint32_t)(vector-32));
	}
	else if ( vector >= 64 )
	{
		NVIC->ICPR[2] = (1 << (uint32_t)(vector-64));
