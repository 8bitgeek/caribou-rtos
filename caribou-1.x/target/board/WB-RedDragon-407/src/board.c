...
caribou_gpio_t led1			= CARIBOU_GPIO_INIT(GPIOA,CARIBOU_GPIO_PIN_6);
caribou_gpio_t led2			= CARIBOU_GPIO_INIT(GPIOA,CARIBOU_GPIO_PIN_4);
caribou_gpio_t led3			= CARIBOU_GPIO_INIT(GPIOB,CARIBOU_GPIO_PIN_6);
caribou_gpio_t led4			= CARIBOU_GPIO_INIT(GPIOC,CARIBOU_GPIO_PIN_6);
caribou_gpio_t rs485_dir	= CARIBOU_GPIO_INIT(GPIOF,CARIBOU_GPIO_PIN_7);
...
void board_idle()
{
	static caribou_tick_t last_time=0L;
	static uint8_t bits=0x00;

	if ( (caribou_timer_ticks()-last_time) >= 10 )
	{
		uint8_t output_bits = (0x0F & (~(bits++)));
		caribou_gpio_write(&led1,output_bits&0x01);
		caribou_gpio_write(&led2,output_bits&0x02);
		caribou_gpio_write(&led3,output_bits&0x04);
		caribou_gpio_write(&led4,output_bits&0x08);
		last_time = caribou_timer_ticks();
	}
}