...
#include <caribou++/cmutex.h>
	...
	caribou_gpio_t led1	= CARIBOU_GPIO_INIT(GPIOE,CARIBOU_GPIO_PIN_12);
	caribou_gpio_t led2	= CARIBOU_GPIO_INIT(GPIOE,CARIBOU_GPIO_PIN_13);
	caribou_gpio_t led3	= CARIBOU_GPIO_INIT(GPIOE,CARIBOU_GPIO_PIN_14);
	caribou_gpio_t led4	= CARIBOU_GPIO_INIT(GPIOE,CARIBOU_GPIO_PIN_15);
#else
...
caribou_gpio_t	mmc_cs		= CARIBOU_GPIO_INIT(GPIOI,CARIBOU_GPIO_PIN_0);
caribou_gpio_t	rs485_dir	= CARIBOU_GPIO_INIT(GPIOG,CARIBOU_GPIO_PIN_15);
...
#if PRODUCT_SYSLOG_STDOUT
	static CARIBOU::CMutex mutex;
#endif
static void debuglog( int lvl,bool timestamp, const char *format, va_list args)
{
	CARIBOU::CString path;
	CARIBOU::CString buf;
	buf.print( format, args );
	if ( timestamp )
	{
		CARIBOU::CDateTime dateTime(caribou_timer_ticks());
		CARIBOU::CString msg;
		msg = msg.sprintf("%03d:%02d:%02d:%02d.%03d",	dateTime.days(),
														dateTime.hours(),
														dateTime.minutes(),
														dateTime.seconds(),
														dateTime.hundredths() );
		...
		switch(lvl)
		{
			case SYSTEM_LVL:	path.append(SYSTEM_LOG);	break;
			case READINGS_LVL:	path.append(READINGS_LOG);	break;
			case TRAFFIC_LVL:	path.append(TRAFFIC_LOG);	break;
			default:
			case DEBUG_LVL:		path.append(DEBUG_LOG);	break;
		}
		path.append(".log");
		CARIBOU::CFile file(path);
		caribou_thread_lock();
		if ( file.open(FA_READ | FA_WRITE | FA_OPEN_ALWAYS) )
		{
			if ( file.size() < (Accutron::CSettings::logMaxSize((uint8_t)lvl)*1024) )
			{
				file.seek(file.size());
...
extern "C" void syslog(int lvl,bool timestamp,const char *format, ...)
{
	va_list args;
	va_start( args, format );
	#if PRODUCT_SYSLOG_ALL
		debuglog(lvl,timestamp,format,args);
	#else
		if ( Accutron::CSettings::logEnabled(lvl) )
		{
			debuglog(lvl,timestamp,format,args);
		}
	#endif
...
void InitializeSPI_MMC()
{
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_4; /* 10 MHz */
	SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;
	SPI_InitStructure.SPI_CRCPolynomial = 7;
	SPI_Init(SPI_MMC,&SPI_InitStructure);
	...
	SPI_MMC->CR1 |= SPI_CR1_SPE;	/* Enable SPI_MMC */
}
...
void late_init()
{
	...
	_stderr = fopen(RS485_UART,"rw");
	uart_config.baud_rate = CARIBOU_UART_BAUD_RATE_9600;
	caribou_uart_set_config(RS485_UART,&uart_config);
	...
	/* Clear the LEDs */
	caribou_gpio_write(&led1,false);
	caribou_gpio_write(&led2,false);
	caribou_gpio_write(&led3,false);
	caribou_gpio_write(&led4,false);

	/* Initialize the MMC Spi Interface */
	InitializeSPI_MMC();
