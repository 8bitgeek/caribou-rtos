...
int caribou_thread_unlock(void)
{
	int rc=0;
	int state = chip_systick_irq_disable();
	if ( caribou_state.current && caribou_state.current->lock > 0 )
	{
		rc = caribou_state.current->lock;
		--caribou_state.current->lock;
	}
...
{
	if ( caribou_state.current && !caribou_state.current->lock && chip_interrupts_enabled() )
	{
...
static void _swap_thread()
{
	...
	if ( preemptable( caribou_state.current ) && caribou_state.priority-- < 0 )
	{
		...
		if ( ( caribou_state.priority = caribou_state.current->prio ) < 0)
		{
			caribou_state.priority = 0;
		}
