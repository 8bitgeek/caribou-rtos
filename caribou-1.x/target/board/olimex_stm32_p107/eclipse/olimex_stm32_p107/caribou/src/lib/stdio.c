...
{
	int fd = _fd(fp);
	while( caribou_uart_tx_busy(fd) || caribou_queue_empty(caribou_uart_tx_queue(fd)) )
	{
...
extern __attribute__((weak)) int debug_printf(const char *format, ...)
{
	int rc=0;
		...
		va_list args;
		va_start( args, format );
		if ( CARIBOU_DEBUG_PRINTF )
		{
			if ( caribou_interrupts_enabled() )
				rc = print(CARIBOU_DEBUG_PRINTF, 0, format, args );
		}
		va_end( args );
	#endif
	return rc;
