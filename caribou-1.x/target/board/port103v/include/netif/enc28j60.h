...
#define PHSTAT2_TXSTAT	(1<<13)		// PHY Transmit Status bit
#define PHSTAT2_RXSTAT	(1<<12)		// PHY Receive Status bit
#define PHSTAT2_COLSTAT	(1<<11)		// PHY Collision Status bit
#define PHSTAT2_LSTAT	(1<<10)		// PHY Link Status bit (non-latching)
#define PHSTAT2_DPXSTAT	(1<<9)		// PHY Duplex Status bit(1)
#define PHSTAT2_PLRITY	(1<<4)		// PLRITY: Polarity Status bit
...
namespace CARIBOU
{
	class CEnc28J60 : public CObject
	{
		public:
			...
			CEnc28J60(CSpi* spi, CByteArray macaddr);
			virtual ~CEnc28J60();
