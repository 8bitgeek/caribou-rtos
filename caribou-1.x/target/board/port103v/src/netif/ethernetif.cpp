...
#define ETH_INTERUPTS (EIE_INTIE|EIE_PKTIE)
#define	LINK_SPEED_OF_YOUR_NETIF_IN_BPS	10000000

static uint8_t input_packet[MAX_FRAMELEN];
...
struct ethernetif 
{
	...
	struct eth_addr * ethaddr;			/* Ethernet MAC */
...
{
    struct ethernetif* ethernetif = static_cast<struct ethernetif*>(netif->state);
	bool mem_okay=false;
	...
	if ( ethernetif )
	{
		...
		if ( ethernetif->ethIf )
		{
			...
			if ( ethernetif->ethInt )
			{
				...
				ethernetif->ethInt->enable();
				ethernetif->ethIf->interruptEnable(ETH_INTERUPTS);
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
	...
	for(q = p; q != NULL; q = q->next) 
	{
        ...
        ethernetif->ethIf->packetSend(static_cast<uint8_t*>(q->payload),q->len);
...
static struct pbuf* low_level_input(struct netif* netif)
{
    ...
    len = ethernetif->ethIf->packetReceive(input_packet,MAX_FRAMELEN);
	...
	if (p != NULL) 
	{
		...
		for(q = p; q != NULL; q = q->next) 
		{
			...
			*/
            if ( !ethernetif->ethIf->packetReceive(static_cast<uint8_t*>(q->payload), q->len) )
				break;
		}
...
err_t ethernetif_init(struct netif *netif)
{
	if ( netif )
	{
		...
		ethernetif = static_cast<struct ethernetif*>(mem_malloc(sizeof(struct ethernetif)));
		if ( ethernetif )
		{
	...
	{
		LWIP_DEBUGF(NETIF_DEBUG, ("netif == NULL\n"));
		return ERR_MEM;
	}
}
