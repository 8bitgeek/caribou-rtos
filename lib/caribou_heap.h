...
extern "C"
{
	...
	/**
	 * @brief CARIBOU_caribou_heap_MPU indicates that the heap region is protected by MPU.
	 */
	#define	CARIBOU_caribou_heap_MPU			0x80000000
	#define	CARIBOU_caribou_heap_REGION_MSK		0x000000FF
...
extern void		caribou_heap_free(void* p);
	...
	extern heap_state_t*	caribou_heap_mpu_init(void* heap_base, uint8_t mpu_region_size);
	extern bool				caribou_heap_mpu_claim(heap_claim_t* claim);
	extern void*			caribou_heap_mpu_claim_malloc(heap_claim_t* claim,size_t size);
	extern void				caribou_heap_mpu_assign(caribou_thread_t* thread, heap_claim_t* claim);
	extern void				caribou_heap_mpu_disable();
	extern void				caribou_heap_mpu_enable();
#endif
