...
#include <caribou_uart.h>
#include <chip/uart.h>

caribou_stdio_t* _stdin=0;
caribou_stdio_t* _stdout=0;
caribou_stdio_t* _stderr=0;
...
static int _atob (unsigned long long *vp, char *p, int base)
{
	...
	if (base == 16 && (q = caribou_strchr (p, '.')) != 0) {
		if (q - p > sizeof(tmp) - 1)
			return (0);

		caribou_strncpy (tmp, p, q - p);
		tmp[q - p] = '\0';
		if (!_atob (&v1, tmp, 16))
			return (0);
 ...
 */
int caribou_atob(uint32_t *vp, char *p, int base)
{
 ...
 *      converts p to binary result in vp, rtn 1 on success
 */
int caribou_llatob(unsigned long long *vp, char *p, int base)
{
	if (base == 0)
		p = _getbase (p, &base);
 ...
 */
char *caribou_btoa(char *dst, uint32_t value, int base)
{
	char buf[34], digit;
	int i, j, rem, neg;
	...
	/* reverse the string */
	for (i = 0, j = caribou_strlen (buf) - 1; j >= 0; i++, j--)
		dst[i] = buf[j];
	dst[i] = 0;
	return (dst);
}
 ...
 *      converts value to ascii, result in dst
 */
char *caribou_llbtoa(char *dst, unsigned long long value, int base)
{
	char buf[66], digit;
	int i, j, rem, neg;
 ...
 *      rtn 1 on success
 */
int caribou_gethex(int32_t *vp, char *p, int n)
{
	unsigned long v;
	int digit;
...
static void caribou_printchar(FILE *fp, char **str, int c)
{
	if (str && *str)
	{
		**str = c;
...
static int caribou_prints(FILE *fp, char **out, const char *string, int width, int pad)
{
	int pc = 0, padchar = ' ';
...
int caribou_scanf(const char *fmt, ...)
{
    ...
    caribou_va_start (ap, fmt);
    count = caribou_vfscanf (stdin, fmt, ap);
    caribou_va_end (ap);
    return (count);
}
...
int caribou_fscanf(FILE *fp, const char *fmt, ...)
{
    ...
    caribou_va_start (ap, fmt);
    count = caribou_vfscanf (fp, fmt, ap);
    caribou_va_end (ap);
    return (count);
}
...
int caribou_sscanf(const char *buf, const char *fmt, ...)
{
    ...
    caribou_va_start (ap, fmt);
    count = caribou_vsscanf (buf, fmt, ap);
    caribou_va_end (ap);
    return (count);
}
...
int caribou_vsscanf(const char *buf, const char *s, va_list ap)
{
	...
	if (*s == '%') {
	    s++;
	    for (; *s; s++) {
		if (caribou_strchr ("dibouxcsefg%", *s))
		    break;
		if (*s == '*')
		    noassign = 1;
		else if (*s == 'l' || *s == 'L')
		    ...
		    caribou_strncpy (tmp, tc, s - tc);
		    tmp[s - tc] = '\0';
		    caribou_atob (&width, tmp, 10);
		    s--;
		}
	    }
	    if (*s == 's') {
		while (isspace (*buf))
		    buf++;
		if (!width)
		    width = caribou_strcspn (buf, ISSPACE);
		if (!noassign) {
		    caribou_strncpy (t = caribou_va_arg (ap, char *), buf, width);
		    t[width] = '\0';
		}
		buf += width;
	    } else if (*s == 'c') {
		...
		if (!width) {
			...
			width = caribou_strchr (buf, *(s + 1)) - buf;
		}
		caribou_strncpy (tmp, buf, width);
		tmp[width] = '\0';
		buf += width;
		if (!noassign)
		    caribou_atob (caribou_va_arg (ap, uint32_t *), tmp, base);
	    }
	    if (!noassign)
		count++;
...
{
	int rc=0;
	caribou_va_list args;
	caribou_va_start( args, format );
	rc = print(NULL, &str, format, args );
	caribou_va_end( args );
	return rc;
}
...
extern __attribute__((weak)) int caribou_debug_printf(const char *format, ...)
{
	int rc=0;
		caribou_va_list args;
		caribou_va_start( args, format );
		{
			if ( caribou_interrupts_enabled() )
				rc = caribou_print(stderr, 0, format, args );
		}
		caribou_va_end( args );
	return rc;
}
...
int caribou_getchar()
{
	return caribou_fgetc(stdin);
}

int caribou_putchar(int ch)
{
	return caribou_fputc(ch,stdout);
}
