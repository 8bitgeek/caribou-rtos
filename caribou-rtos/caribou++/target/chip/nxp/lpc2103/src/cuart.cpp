...
#define DEFAULT_RX_BUFFERSZ	32
#define DEFAULT_TX_BUFFERSZ	32
...
typedef struct
{
	...
	CARIBOU::CObject::Vector	vector;			// the interrupt vector for the port.
...
static port_t ports[] =
{
	{ (uint32_t*)UART0_BASE, (uint32_t*)0xE002C000, CARIBOU::CObject::Vector_UART0, (U0IER_RBR_Interrupt_Enable | U0IER_THRE_Interrupt_Enable), 0x00000005 },
	{ (uint32_t*)UART1_BASE, (uint32_t*)0xE002C010, CARIBOU::CObject::Vector_UART1, (U1IER_RBR_Interrupt_Enable | U1IER_THRE_Interrupt_Enable), 0x00050000 },
};

namespace CARIBOU
{
	#define inherited CUart_Base

	CUart::CUart(int port, uint32_t baud, uint32_t mode, uint16_t stksize, uint16_t priority )
	: inherited()
	, mPort(port)
	...
	* Open the device.
	*/
	bool CUart::open(CIODevice::IOMode mode)
	{
		setMode(mode);
		enable();
	}
	...
	*/
	 ...
	 **/
	void CUart::irq(CObject::Vector v)
	{
	...
	*/
	...
	//*****************************************************************************
	uint32_t CUart::status(bool bMasked)
	{
	...
	//! asserted).
	//*****************************************************************************
	void CUart::clear(uint32_t ulIntFlags)
	{
	...
	//
	//*****************************************************************************
	bool CUart::txNonBlock(uint8_t ucData)
	{
		if ( uartTxReady() )
		{
	...
	//
	//*****************************************************************************
	void CUart::setConfig(uint32_t ulUARTClk, uint32_t ulBaud, uint32_t ulConfig)
	{
	...
	//
	//*****************************************************************************
	void CUart::getConfig(uint32_t *pulBaud, uint32_t *pulConfig)
	{
		/** FIXME */
		*pulBaud=9600;
		*pulConfig=0;
	...
	//
	//*****************************************************************************
	void CUart::enableInt(uint32_t ulIntFlags)
	{
	...
	//
	//*****************************************************************************
	void CUart::disableInt(uint32_t ulIntFlags)
	{
		*(volatile uint32_t*)mIER &= ~ulIntFlags;
	}
