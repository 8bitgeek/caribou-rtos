...
#define netifGUARD_BLOCK_TIME				TIMEOUT_INFINITE
...
static void low_level_init(struct netif *netif)
{
	...
	uint8_t* ethifStack = (uint8_t*)malloc(netifINTERFACE_TASK_STACK_SIZE);
	if (ethifStack )
	{
...
static struct pbuf * low_level_input(struct netif *netif)
{
	...
	if ((frame.descriptor->Status & ETH_DMARxDesc_ES) == (uint32_t)RESET)
	{
		...
		/* We allocate a pbuf chain of pbufs from the pool. */
			...
			while(!completed)
			{
				/** Safely aloocate a pbuf */
				int state = caribou_interrupts_disable();
				if ( !__lwip_core_lock__ )
				{
		...
		while ( ethInputCount > 0 )
		{
			...
			if ( (p = low_level_input( s_pxNetIf )) )
			{
				if (ERR_OK != s_pxNetIf->input( p, s_pxNetIf))
				{
						...
						while(!completed)
						{
							/** Safely test the lwip core lock... */
							int state = caribou_interrupts_disable();
							if ( !__lwip_core_lock__ )
							{
