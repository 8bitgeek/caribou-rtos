...
void thread1(void* arg)
{
	...
	static bool pb1_active=true;
	static bool pb2_active=true;
	...
	for(;;)
	{
		...
		{
			pb1_active = !pb1_active;
			while( caribou_gpio_pinstate(&pb1) )
			{
				caribou_gpio_reset(&led1);
				caribou_gpio_reset(&led2);
			}
		...
		if ( !caribou_gpio_pinstate(&pb2) )
		{
			pb2_active = !pb2_active;
			while( !caribou_gpio_pinstate(&pb2) )
			{
				caribou_gpio_reset(&led1);
				caribou_gpio_reset(&led2);
			}
