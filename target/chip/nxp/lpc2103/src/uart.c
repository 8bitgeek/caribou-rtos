...
{
	int rc=(-1);
	static volatile caribou_uart_config_t* pconfig;
	chip_uart_private_t* private_device = (chip_uart_private_t*)device;
	if ( !config )
	{
    ...
    volatile unsigned long ulDivisor = peripheralClockFrequency() / (16 * (int)config->baud_rate);
    ...
    *((volatile uint8_t*)((uint8_t*)BASE(private_device)+U0FCR_OFFSET)) =     /* Enable FIFOs and reset them. */
	(U0FCR_Rx_Trigger_Level_Select_MASK & (3 <<U0FCR_Rx_Trigger_Level_Select_BIT)) | 
	U0FCR_FIFO_Enable |
	U0FCR_Rx_FIFO_Reset | 
	U0FCR_Tx_FIFO_Reset; 
#else
...
void isr_uart(InterruptVector vector,void* arg)
{
    ...
    while ( ((rIIR=(*(volatile uint32_t*)IIR(device))) & U0IIR_Interrupt_Pending) == 0 )
    {
       switch ( (rIIR>>1)&0x07 )
        {
