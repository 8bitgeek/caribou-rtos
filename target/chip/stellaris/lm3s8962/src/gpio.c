...
{
	uint32_t bits = GPIOPinRead(port,pinmask) ^ pinmask;
    GPIOPinWrite(port,pinmask,bits);
}

void chip_gpio_set(chip_gpio_port_t port, chip_gpio_pinmask_t pinmask)
{
    GPIOPinWrite(port,pinmask,pinmask);
}
...
{
	chip_gpio_pinmask_t rc = GPIOPinRead(port,pinmask);
	return rc;
}
