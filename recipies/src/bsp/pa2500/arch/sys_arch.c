...
{
	caribou_mutex_lock(pval,0);
}

void sys_arch_unprotect(sys_prot_t pval)
{
	caribou_mutex_unlock(pval);
}
...
sys_thread_t sys_thread_new(const char *name, lwip_thread_fn thread, void *arg, int stacksize, int prio)
{
	...
	if (newthread)
	{
		...
		if ( stack )
		{
			...
			#if defined __CARIBOU_10
				newthread->thread = caribou_thread_create(name,thread,NULL,arg,stack,stacksize,prio,0);
			#else
...
{
	err_t rc=ERR_MEM;
	*mutex = caribou_mutex_new(CARIBOU_MUTEX_F_RECURSIVE);
	if (*mutex!=NULL)
	{
		rc=ERR_OK;
	}
