 ...
 * @param[in] directory Handle that identifies the directory
 ...
 * @param[in] directory Handle that identifies the directory to be closed
 **/

void fsDirClose(FsDir *directory)
{
   //Make sure the directory pointer is valid
   if(directory != NULL)
   {
      ...
      f_closedir((DIR *) directory);
      //Leave critical section
      osMutexRelease(fsMutex);
#endif
      //Mark the corresponding entry as free
      ((DIR *) directory)->fs = NULL;
   }
}
