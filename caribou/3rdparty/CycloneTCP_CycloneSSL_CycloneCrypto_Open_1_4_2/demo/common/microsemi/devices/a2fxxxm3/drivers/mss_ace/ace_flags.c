 ...
 * 
 * SVN $Revision: 3951 $
 * SVN $Date: 2011-11-14 11:36:55 +0000 (Mon, 14 Nov 2011) $
 */
#include "mss_ace.h"
    ...
    if ( flag_handle < NB_OF_ACE_FLAG_HANDLES )
    {
        ...
        if ( ppe_flag_group < NB_OF_FLAG_GROUPS )
        {
            ...
            *(flags_irq_enable_regs_lut[ppe_flag_group]) &= (uint32_t)~flag_id_mask;
            /*
             * Ensure that the posted write to the PPE_FLAGSn_IRQ_CLR register
             * completed before returning from this function. Not doing this may
             * result in the interrupt only being disabled some time after this
             * function returns.
             */
    ...
    if ( flag_handle < NB_OF_ACE_FLAG_HANDLES )
    {
        ...
        if ( ppe_flag_group < NB_OF_FLAG_GROUPS )
        {
            ...
            *(flags_irq_clear_regs_lut[ppe_flag_group]) |= flag_id_mask;
             ...
             * Ensure that the posted write to the PPE_FLAGSn_IRQ_CLR register
             * completed before returning from this function. Not doing this may
             * result in the interrupt retriggering if the Cortex-M3 returns from
             * interrupt before the posted write completes.
             */
