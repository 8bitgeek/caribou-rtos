...
#define PLL0_M 100
//Divide value for creating the CPU clock (CCLK)
#define CCLK_DIV 4
...
{
   //Flash access timing
   LPC_SC->FLASHCFG = FLASHCFG_FLASHTIM_5CLK | FLASHCFG_RESET_VALUE;

   //Select the frequency range (1MHz to 20MHz)
   LPC_SC->SCS &= ~SCS_OSCRANGE;
   //Enable main oscillator
   LPC_SC->SCS = SCS_OSCEN;
   //Ensure the oscillator is running and stable
   ...
   LPC_SC->CLKSRCSEL = CLKSRCSEL_CLKSRC_OSC;
   //Set CPU clock
   LPC_SC->CCLKCFG = (CCLK_DIV - 1);
   //Set peripheral clock to CCLK/4
   LPC_SC->PCLKSEL0 = 0x00000000;
   LPC_SC->PCLKSEL1 = 0x00000000;

   //Configure PLL0
   LPC_SC->PLL0CFG = ((PLL0_N - 1) << 16) | (PLL0_M - 1);
   LPC_SC->PLL0FEED = 0xAA;
   LPC_SC->PLL0FEED = 0x55;
   //Enable PLL0
   LPC_SC->PLL0CON |= PLL0CON_PLLE0;
   ...
   while(!(LPC_SC->PLL0STAT & PLL0STAT_PLOCK0));
   //Connect PLL0
   LPC_SC->PLL0CON |= PLL0CON_PLLC0;
   LPC_SC->PLL0FEED = 0xAA;
   LPC_SC->PLL0FEED = 0x55;
   ...
   LPC_SC->CLKOUTCFG &= ~CLKOUTCFG_CLKOUT_EN;
